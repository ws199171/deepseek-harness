...
| [`llm-cli/`](llm-cli/README.md) | Delegates each model call to a CodeBuddy CLI child process, so a deployment can answer requests without an API key | registers on `ctx.llm` |

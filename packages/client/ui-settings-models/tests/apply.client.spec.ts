...
import { unavailableCliRemote } from '../src/client/CliProviderPanel.tsx'
...
async function bench(isLoopback = true, mock = RemoteMock.create().load(remoteDefaultResponses), services: object = {}) {
  ...
  const remote = new TestRemote(ctx, {
    ...
    // The CLI discovery namespace is a dotted inject service; the panel's own
    // unavailable face stands in so no test depends on the real Remote mount.
    'llm-cli-discovery': unavailableCliRemote(),
...
describe('ui-settings-models apply', () => {
  ...
  it('declares the services it uses', () => {
    expect(inject).toEqual([
      ...
      'settingsScope', 'settingsSchema', 'remote.llm-cli-discovery',
    ])
  })
